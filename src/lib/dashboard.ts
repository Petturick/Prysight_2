...
export function deriveProductMetrics(product: ProductWithRelations, filters: DashboardFilters = {}) {
  ...
  const comparisonOwnPrice = ownPrice !== null && !(selectedMarket?.vatIncluded ?? product.vatIncluded) && vatRate !== null
  ...
  return {
    ...
    comparisonOwnPrice,
    ownCurrency,
    vatIncluded: selectedMarket?.vatIncluded ?? product.vatIncluded,
    lowestPrice,
    averagePrice,
    difference,
