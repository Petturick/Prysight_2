...
export type FeedTargetKey = (typeof FEED_TARGET_FIELDS)[number]['key']

const FIELD_ALIASES: Record<string, string[]> = {
  articleNumber: [
    'articlenumber', 'article_number', 'artikelnummer', 'artikel_nummer', 'artikel', 'sku', 'sku_code', 'skucode',
    'productsku', 'product_sku', 'itemnumber', 'item_number', 'itemno', 'productcode', 'product_code', 'productid',
    'product_id', 'merchantproductid', 'merchant_product_id', 'sku_new', 'sku_old', 'nieuweartikelnummer', 'artikelnummernieuw',
  ],
...
export function inferImportMapping(headers: string[]) {
  ...
  const mapping = Object.fromEntries(allowed.map((key) => [key, ''])) as Record<ImportTargetKey, string>
...
export function inferFeedMapping(headers: string[], sample: Record<string, string> = {}) {
  const allowed = FEED_TARGET_FIELDS.map((field) => field.key)
  return headers.map((sourceColumn) => ({
    sourceColumn,
    targetField: inferHeaderTarget(sourceColumn, allowed) || null,
