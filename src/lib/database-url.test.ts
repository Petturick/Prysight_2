...
test('prefers canonical PRICING_DB project credentials over an older DATABASE_URL', () => {
  const source = 'postgresql://postgres.oldproject:old@aws-0-eu-west-2.pooler.supabase.com:5432/postgres?sslmode=require'
...
test('uses explicit Supabase project configuration when DATABASE_URL is absent', () => {
  const result = resolveDatabaseConnection(
    '',
    'eu-west-2',
    'xmedaatjwxkmwkjmwuuz',
...
test('falls back to DATABASE_URL when canonical project credentials are unavailable', () => {
  const source = 'postgresql://postgres.oldproject:old@aws-0-eu-west-2.pooler.supabase.com:5432/postgres?sslmode=require'
  const result = resolveDatabaseConnection(source, 'eu-west-2', '', '')
  const url = new URL(result.connectionString)
  assert.equal(result.mode, 'supavisor')
  assert.equal(decodeURIComponent(url.username), 'postgres.oldproject')
  assert.equal(decodeURIComponent(url.password), 'old')
})
