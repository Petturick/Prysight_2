...
async function createAlertOnce({ companyId, type, productId, competitorOfferId, title, message, severity }: { companyId: string; type: string; productId?: string | null; competitorOfferId?: string | null; title: string; message: string; severity: AlertSeverity }) {
  const since = new Date(Date.now() - 12 * 60 * 60 * 1000)
  const existing = await prisma.alert.findFirst({ where: { companyId, type, productId: productId ?? undefined, competitorOfferId: competitorOfferId ?? undefined, createdAt: { gte: since }, isRead: false } })
  if (existing) return existing

  const alert = await prisma.alert.create({ data: { companyId, type, productId: productId ?? null, competitorOfferId: competitorOfferId ?? null, title, message, severity } })
...
export async function evaluateMonitoringAlerts({ companyId, competitorOfferId, competitorId, countryId, productId, productGroupId, competitorName, productName, previousPrice, currentPrice, ownPrice, previousStockStatus, currentStockStatus }: { companyId: string; competitorOfferId: string; competitorId: string; countryId: string; productId?: string | null; productGroupId?: string | null; competitorName: string; productName: string; previousPrice: Prisma.Decimal | number | string | null | undefined; currentPrice: Prisma.Decimal | number | string | null | undefined; ownPrice: Prisma.Decimal | number | string | null | undefined; previousStockStatus?: string | null; currentStockStatus?: string | null }) {
  ...
  if (own && current && current > 0) {
    ...
    if (priceIndex >= maxIndex) await createAlertOnce({ companyId, type: 'PRICE_GAP', productId, competitorOfferId, title: 'Eigen prijs ligt boven de markt', message: `${productName} staat ${Math.abs(priceIndex - 100).toFixed(1)}% boven ${competitorName}.`, severity: priceIndex >= maxIndex + 5 ? AlertSeverity.CRITICAL : AlertSeverity.WARNING })
    ...
    if (competitorIndex >= opportunityThreshold) await createAlertOnce({ companyId, type: 'PRICE_OPPORTUNITY', productId, competitorOfferId, title: 'Mogelijke ruimte voor prijsverhoging', message: `${competitorName} ligt ${Math.abs(competitorIndex - 100).toFixed(1)}% boven de eigen prijs van ${productName}.`, severity: AlertSeverity.INFO })
  }

  if (previousStockStatus && currentStockStatus && previousStockStatus !== currentStockStatus) await createAlertOnce({ companyId, type: 'STOCK_CHANGE', productId, competitorOfferId, title: 'Voorraadstatus concurrent gewijzigd', message: `${competitorName} wijzigde de voorraadstatus van ${productName} van ${previousStockStatus} naar ${currentStockStatus}.`, severity: AlertSeverity.INFO })
}
