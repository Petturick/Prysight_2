...
test('Product toevoegen heeft één compacte primaire route en verbergt alleen optionele velden', () => {
  ...
  assert.match(page, /Prijs inclusief btw \*/)
  assert.match(page, /Prijs exclusief btw/)
  assert.match(page, /Verzendkosten inclusief btw/)
  assert.ok(page.indexOf('Prijs inclusief btw *') < page.indexOf('Prijs exclusief btw'), 'prijs inclusief btw moet boven prijs exclusief btw staan')
