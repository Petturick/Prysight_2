...
export default async function FeedDiagnosePage() {
  ...
  const result = await safeDatabaseQuery(async () => {
    const [sources, runs] = await Promise.all([
      prisma.feedSource.findMany({ where: { companyId: actor.companyId }, orderBy: { name: 'asc' } }),
      prisma.feedSyncRun.findMany({ where: { companyId: actor.companyId }, include: { feedSource: true }, orderBy: { startedAt: 'desc' }, take: 30 }),
    ])
    return { sources, runs }
  }, { sources: [], runs: [] })
