    ...
    <div className="space-y-5">
      ...
      <section className="strong-panel overflow-hidden">
        <div className="flex flex-col gap-4 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-6">
          <div><p className="eyebrow">Data en beheer</p><h1 className="mt-2">Feedbeheer</h1><p className="mt-2 max-w-3xl text-[12px] font-medium leading-6 text-[#4b5870]">Beheer alle productbronnen van de actieve organisatie vanuit één plek en zie direct welke bron actief is, faalt of aandacht vraagt.</p></div>
      ...
      <section id="externe-feed" className="strong-panel scroll-mt-5 overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-[var(--border-strong)] bg-[#edf1f6] px-5 py-4"><div><h2 className="text-[15px] font-black text-[#111827]">Externe productfeed koppelen</h2><p className="mt-1 text-[11px] font-semibold text-[#647087]">Bron ophalen, velden herkennen, preview controleren en pas daarna producten bijwerken.</p></div><a href="#bronbeheer" className="secondary-action min-h-0 px-3 py-2">Bestaande bronnen beheren</a></div><div className="p-5"><FeedConnectForm disabled={!result.available || !canManage} /></div></section>
      ...
      <section className="surface-card overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-[var(--border-strong)] bg-[#111827] px-5 py-4 text-white"><div><h2 className="text-[15px] font-black">Recente databronnen</h2><p className="mt-1 text-[11px] font-medium text-[#cbd5e1]">De laatste bronnen van deze organisatie die productdata aan Prysight hebben geleverd.</p></div><span className="rounded-[8px] bg-white/10 px-3 py-1 text-[10px] font-black">{formatNumber(Math.min(sources.length, 8))} zichtbaar</span></div><div className="divide-y-2 divide-[#cbd2df]">{sources.length === 0 ? <p className="py-10 text-center text-[11px] font-semibold text-[#6f7b91]">Nog geen databron gekoppeld.</p> : sources.slice(0, 8).map((source) => <div key={source.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 odd:bg-white even:bg-[#f3f5f9]"><div><div className="flex flex-wrap items-center gap-2"><p className="text-[12px] font-black text-[#111827]">{source.name}</p><span className="rounded-[7px] border border-[#aeb8c9] bg-white px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.06em] text-[#4b5870]">{source.sourceType}</span></div><p className="mt-1 text-[10px] font-semibold text-[#6f7b91]">{source.countryCode || 'GLOBAL'} · {source.format ?? 'formaat wordt bepaald'} · laatste run {formatDate(source.lastRunAt)}</p></div><div className="text-right"><p className="text-[12px] font-black text-[#111827]">{formatNumber(source.lastItemCount)} regels</p><p className={`mt-1 inline-flex rounded-[7px] px-2 py-1 text-[9px] font-black uppercase ${source.lastRunStatus === 'FAILED' ? 'bg-[#b4233d] text-white' : source.lastRunStatus === 'COMPLETED' ? 'bg-[#0d7a49] text-white' : 'bg-[#dfe4ee] text-[#334155]'}`}>{source.lastRunStatus}</p></div></div>)}</div></section>
    </div>
  )
}
