...
export async function GET(request: Request) {
  try {
    ...
    const result = await getPricingRecommendations(actor.companyId, { strategy, adjustmentPct: numberParam(searchParams.get('adjustmentPct'), 0), maxChangePct: Math.max(0, numberParam(searchParams.get('maxChangePct'), 5)), minimumSignalPct: Math.max(0, numberParam(searchParams.get('minimumSignalPct'), 1)), onlyInStock: searchParams.get('onlyInStock') !== 'false' }, Math.min(Math.max(numberParam(searchParams.get('limit'), 200), 1), 500))
