...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const columns = await prisma.feedColumnMapping.findMany({ where: { companyId: DEFAULT_COMPANY_ID, feedSourceId: id, id: { in: columnIds } }, select: { id: true } })
  ...
  await prisma.$transaction(body.mappings.map((mapping) => prisma.feedColumnMapping.update({ where: { id: mapping.id as string }, data: { targetField: mapping.targetField?.trim() || null } })))
  return NextResponse.json({ ok: true })
}

export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  const { id } = await context.params
  const source = await prisma.feedSource.findFirst({ where: { id, companyId: DEFAULT_COMPANY_ID }, include: { columns: { orderBy: { position: 'asc' } } } })
  ...
  await prisma.$transaction(suggestions.map((suggestion) => prisma.feedColumnMapping.update({ where: { id: suggestion.id }, data: { targetField: suggestion.target } })))
