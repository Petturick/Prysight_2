...
export async function POST(request: Request) {
  ...
  let normalized: URL
  try { normalized = validateFeedUrl(body.url) } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Ongeldige feed URL.' }, { status: 400 })
  }
  ...
  const source = await prisma.feedSource.upsert({
    where: { companyId_sourceKey: { companyId: DEFAULT_COMPANY_ID, sourceKey: sourceKey(normalized.toString()) } },
    update: { name, url: normalized.toString(), countryCode, isActive: true, syncError: null },
    create: { companyId: DEFAULT_COMPANY_ID, sourceKey: sourceKey(normalized.toString()), name, sourceType: FeedSourceType.URL, url: normalized.toString(), countryCode, isActive: true },
  })
