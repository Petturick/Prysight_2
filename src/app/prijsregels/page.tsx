...
function fieldClass() {
  return 'h-10 w-full rounded-xl border-2 border-[#d5dce7] bg-white px-3 text-[12px] font-semibold text-[#2e3748] outline-none transition focus:border-[#5879ac]'
}
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="surface-card overflow-hidden">
          <div className="border-b-2 border-[var(--border-strong)] bg-[#111827] px-5 py-4 text-white">
            <h2 className="text-[15px] font-black">{editRule ? 'Prijsregel bewerken' : 'Nieuwe prijsregel'}</h2>
            <p className="mt-1 text-[11px] font-medium text-[#cbd5e1]">Een specifiekere regel wint van een algemene regel. Bij gelijke specificiteit wint de hoogste prioriteit.</p>
          </div>
          <form action={savePricingRuleAction} className="grid gap-4 p-5 sm:grid-cols-2">
            ...
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Correctie procent</span><input name="adjustmentPct" type="number" step="0.1" defaultValue={editRule?.adjustmentPct ?? 0} className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Max wijziging procent</span><input name="maxChangePct" type="number" min="0" max="100" step="0.1" defaultValue={editRule?.maxChangePct ?? 5} className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Signaaldrempel procent</span><input name="minimumSignalPct" type="number" min="0" max="100" step="0.1" defaultValue={editRule?.minimumSignalPct ?? 1} className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Minimaal concurrenten</span><input name="minimumCompetitors" type="number" min="1" step="1" defaultValue={editRule?.minimumCompetitors ?? 2} className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Minimale marge procent</span><input name="minimumMarginPct" type="number" min="0" max="99.9" step="0.1" defaultValue={money(editRule?.minimumMarginPct)} placeholder="Optioneel" className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Minimumprijs</span><input name="minimumPrice" type="number" min="0" step="0.01" defaultValue={money(editRule?.minimumPrice)} placeholder="Optioneel" className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Maximumprijs</span><input name="maximumPrice" type="number" min="0" step="0.01" defaultValue={money(editRule?.maximumPrice)} placeholder="Optioneel" className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Prioriteit</span><input name="priority" type="number" step="1" defaultValue={editRule?.priority ?? 0} className={fieldClass()} /></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Voorraadfilter</span><select name="onlyInStock" defaultValue={String(editRule?.onlyInStock ?? true)} className={fieldClass()}><option value="true">Alleen op voorraad</option><option value="false">Alle actieve aanbiedingen</option></select></label>
            <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Goedkeuring</span><select name="requireApproval" defaultValue={String(editRule?.requireApproval ?? true)} className={fieldClass()}><option value="true">Altijd goedkeuren</option><option value="false">Mag later automatisch</option></select></label>
        ...
        <div className="surface-card overflow-hidden">
          <div className="border-b-2 border-[var(--border-strong)] bg-[#111827] px-5 py-4 text-white"><h2 className="text-[15px] font-black">Productguardrails</h2><p className="mt-1 text-[11px] font-medium text-[#cbd5e1]">Deze waarden zijn harder dan iedere prijsregel. Kostprijs is netto, exclusief btw.</p></div>
          <form className="border-b-2 border-[var(--border)] p-5"><label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Product laden</span><div className="flex gap-2"><select name="productId" defaultValue={selectedProductId} className={fieldClass()}><option value="">Kies een product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.articleNumber} · {product.name}</option>)}</select><button className="secondary-action" type="submit">Laden</button></div></label></form>
          {selectedProduct ? (
            <form action={saveProductGuardrailAction} className="grid gap-4 p-5 sm:grid-cols-2">
              <input type="hidden" name="productId" value={selectedProduct.id} />
              <div className="sm:col-span-2 rounded-xl border-2 border-[#d5dce7] bg-[#f5f7fa] px-4 py-3"><p className="text-[12px] font-black text-[#172033]">{selectedProduct.articleNumber} · {selectedProduct.name}</p></div>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Kostprijs netto</span><input name="costPrice" type="number" min="0" step="0.01" defaultValue={money(selectedGuardrail?.costPrice)} className={fieldClass()} /></label>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Minimale marge procent</span><input name="minimumMarginPct" type="number" min="0" max="99.9" step="0.1" defaultValue={money(selectedGuardrail?.minimumMarginPct)} className={fieldClass()} /></label>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Minimumprijs</span><input name="minimumPrice" type="number" min="0" step="0.01" defaultValue={money(selectedGuardrail?.minimumPrice)} className={fieldClass()} /></label>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Maximumprijs</span><input name="maximumPrice" type="number" min="0" step="0.01" defaultValue={money(selectedGuardrail?.maximumPrice)} className={fieldClass()} /></label>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Doelmarge procent</span><input name="targetMarginPct" type="number" min="0" max="99.9" step="0.1" defaultValue={money(selectedGuardrail?.targetMarginPct)} className={fieldClass()} /></label>
              <label><span className="mb-1.5 block text-[10px] font-black uppercase tracking-[0.06em] text-[#68758a]">Productafronding</span><select name="priceRoundingMode" defaultValue={selectedGuardrail?.priceRoundingMode ?? 'CENT'} className={fieldClass()}>{Object.entries(roundingLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      ...
      <section className="surface-card overflow-hidden">
        <div className="border-b-2 border-[var(--border-strong)] bg-[#111827] px-5 py-4 text-white"><h2 className="text-[15px] font-black">Actieve prijsregels</h2><p className="mt-1 text-[11px] font-medium text-[#cbd5e1]">{rules.length} regel{rules.length === 1 ? '' : 's'} actief voor deze organisatie.</p></div>
        {rules.length === 0 ? <div className="p-5 text-[12px] font-semibold text-[#68758a]">Nog geen prijsregels. Zonder regel gebruikt PrySight de veilige standaardstrategie.</div> : (
          <div className="overflow-x-auto"><table className="min-w-full text-left text-[11px]"><thead className="border-b-2 border-[var(--border)] bg-[#f4f6f9] text-[#5d687d]"><tr><th className="px-4 py-3 font-black">Regel</th><th className="px-4 py-3 font-black">Scope</th><th className="px-4 py-3 font-black">Strategie</th><th className="px-4 py-3 font-black">Guardrails</th><th className="px-4 py-3 font-black">Goedkeuring</th><th className="px-4 py-3 font-black">Acties</th></tr></thead><tbody>{rules.map((rule) => {
