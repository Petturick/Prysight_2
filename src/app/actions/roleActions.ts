...
import { PERMISSIONS, type Permission, requireSuperAdmin } from '@/lib/authz'
...
export async function updateCustomRoleAction(formData: FormData) {
  ...
  if (!roleId || name.length < 2 || permissions.length === 0) {
    throw new Error('Controleer de rol en de geselecteerde rechten.')
  }
...
export async function assignCustomRoleAction(formData: FormData) {
  ...
  if (userId === actor.id && roleId) {
    throw new Error('Het eigen Super Admin account behoudt altijd volledige platformrechten.')
  }
  ...
  if (membership.user.isSuperAdmin) {
    throw new Error('Een Super Admin account kan niet aan een aangepaste organisatierol worden gekoppeld.')
  }
  ...
  await prisma.$executeRaw`
    ...
    SET custom_role_id = ${roleId || null}, updated_at = now()
...
export async function deleteCustomRoleAction(formData: FormData) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.$executeRaw`
      UPDATE company_memberships
      SET custom_role_id = NULL, updated_at = now()
      WHERE custom_role_id = ${roleId} AND company_id = ${actor.companyId}
    `
    await tx.$executeRaw`
