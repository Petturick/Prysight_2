...
export default async function GebruikersPage() {
  ...
  const assignableRoles = Object.values(UserRole).filter((role) => actor.role === 'SUPER_ADMIN' || role !== UserRole.ADMIN)
  ...
  `
  return <div className="space-y-5"><form action={saveUserAction} className="strong-panel grid gap-3 p-5 md:grid-cols-4"><div className="md:col-span-4"><h2 className="text-[15px] font-bold text-[#252a37]">Gebruiker toevoegen</h2><p className="mt-1 text-[10px] text-[#8790a2]">Gebruikers worden uitsluitend aan de actieve organisatie gekoppeld.</p></div><input name="email" type="email" placeholder="E mail" className="toolbar-control" required /><input name="name" placeholder="Naam" className="toolbar-control" required /><input name="password" type="password" minLength={12} placeholder="Wachtwoord" className="toolbar-control" required /><select name="role" className="toolbar-control">{assignableRoles.map((role) => <option key={role} value={role}>{role}</option>)}</select><button className="primary-action md:col-span-4">Gebruiker opslaan</button></form>
