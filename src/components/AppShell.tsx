        ...
        <header className="relative z-20 border-b border-[#e7ebf0] bg-white/95 backdrop-blur-xl">
          <div className="flex min-h-[76px] items-center justify-between px-5 py-4 sm:px-7 lg:px-8">
            ...
            <div className="ml-6 flex shrink-0 items-center gap-2.5">
              ...
              {context.action ? <Link href={context.action.href} prefetch={false} onMouseEnter={() => warm(context.action!.href)} onFocus={() => warm(context.action!.href)} className="hidden min-h-10 items-center gap-2 rounded-[11px] bg-[#172033] px-4 text-[13px] font-semibold tracking-[-0.01em] text-white shadow-[0_4px_10px_rgba(16,24,40,.12)] transition-all hover:-translate-y-px hover:bg-[#25324a] hover:shadow-[0_7px_14px_rgba(16,24,40,.16)] md:inline-flex"><span>{context.action.label}</span><svg className="h-3.5 w-3.5 opacity-75" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg></Link> : null}
            </div>
          </div>
          <nav className="flex gap-1 overflow-x-auto border-t border-[#eef1f4] bg-white px-3 py-2 lg:hidden" aria-label="Hoofdnavigatie">
            {mobileItems.map(([href,label]) => <Link key={href} href={href} prefetch={false} onMouseEnter={() => warm(href)} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${pathname === href || pathname.startsWith(`${href}/`) ? 'bg-[#172033] text-white' : 'text-[#667085] hover:bg-[#f4f6f8] hover:text-[#172033]'}`}>{label}</Link>)}
          </nav>
        </header>
