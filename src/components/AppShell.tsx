        ...
        <header className="sticky top-0 z-20 border-b border-[#edf0f5] bg-white/98 backdrop-blur">
          <div className="flex min-h-[68px] items-center justify-between gap-5 px-4 sm:px-6 lg:px-8 2xl:px-9">
            <form action="/producten" className="relative w-full max-w-[490px]">
              ...
              <input name="q" aria-label="Zoeken" placeholder="Zoek product, EAN of concurrent..." className="h-[42px] w-full rounded-[8px] border border-[#e0e6ef] bg-white pl-10 pr-4 text-[12px] text-[#1b2a41] shadow-[0_2px_7px_rgba(23,39,65,.04)] placeholder:text-[#8b99ae] focus:border-[#90b8f7] focus:ring-0" />
            </form>
            <div className="flex items-center gap-2 sm:gap-4">
              <Link href="/onboarding" prefetch={false} className="hidden rounded-[8px] border border-[#dfe6ef] bg-white px-3 py-2 text-[10px] font-semibold text-[#526177] transition hover:bg-[#f7f9fc] md:inline-flex">Setup</Link>
              <Link href="/acties" prefetch={false} className="relative flex h-9 w-9 items-center justify-center rounded-full text-[#6f7f95] hover:bg-[#f2f5f9]" title="Acties"><svg className="h-[18px] w-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9"><path d="M6 9a6 6 0 0 1 12 0c0 6 2 6 2 8H4c0-2 2-2 2-8Z"/><path d="M10 20h4"/></svg><span className="absolute right-1 top-1 h-2 w-2 rounded-full bg-[#ef6769] ring-2 ring-white" /></Link>
          ...
          <div className="flex gap-1 overflow-x-auto border-t border-[#f0f2f6] px-3 py-2 lg:hidden">{mobileItems.map(([href,label]) => <Link key={href} href={href} prefetch={false} className={`shrink-0 rounded-[7px] border px-3 py-2 text-[10px] font-semibold ${pathname === href || pathname.startsWith(`${href}/`) ? 'border-[#4f86e8] bg-[#4f86e8] text-white' : 'border-[#dce3ec] bg-white text-[#5f6e84]'}`}>{label}</Link>)}</div>
        </header>
