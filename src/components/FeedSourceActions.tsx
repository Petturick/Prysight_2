...
export function FeedSourceActions({ id, isActive, canSync = true }: { id: string; isActive: boolean; canSync?: boolean }) {
  ...
  const call = async (action: 'sync' | 'toggle' | 'delete') => {
    ...
    try {
      ...
      if (action === 'sync') {
        if (result.accepted) {
          setNotice('Synchronisatie gestart. Prysight verwerkt de productfeed op de achtergrond en zoekt daarna automatisch concurrenten voor de juiste markt.')
          router.refresh()
        ...
        const parts = [
          result.rows !== undefined ? `${result.rows} productregels bijgewerkt` : null,
        ...
        setNotice(parts.length ? parts.join(', ') + '.' : 'Synchronisatie afgerond.')
      }
      router.refresh()
    } catch (error) {
