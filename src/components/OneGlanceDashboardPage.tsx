...
export default async function OneGlanceDashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  try {
    snapshot = await profileStep('/dashboard', 'snapshot', () => getDashboardSnapshot(filters, actor.companyId), { companyId: actor.companyId }, 350)
  } catch (error) {
  ...
  return <div className="space-y-4">
    ...
    <section className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]"><div className="strong-panel p-5"><div className="flex items-center justify-between gap-3"><div><p className="eyebrow">Direct handelen</p><h2 className="mt-1 text-[17px] font-bold text-[#243247]">Belangrijkste prijsbewegingen</h2></div><Link href="/producten" prefetch className="secondary-action min-h-0 px-3 py-2">Alle producten</Link></div><div className="mt-4 space-y-2">{priceMovements.length === 0 ? <div className="rounded-[9px] border border-[#dde3e9] bg-[#f8fafc] px-4 py-5 text-[11px] font-medium text-[#697789]">Nog geen recente prijsbewegingen beschikbaar.</div> : priceMovements.map((item, index) => { const down = item.delta < 0; return <div key={`${item.productName}-${item.competitor}-${index}`} className="flex flex-wrap items-center justify-between gap-3 rounded-[9px] border border-[#dde3e9] bg-white px-4 py-3 transition hover:bg-[#f8fafc]"><div><p className="text-[12px] font-semibold text-[#2d3a4d]">{item.productName}</p><p className="mt-1 text-[10px] font-medium text-[#718092]">{item.competitor}</p></div><div className="text-right"><p className={`text-[14px] font-bold ${down ? 'text-[#96313b]' : 'text-[#126a49]'}`}>{down ? '' : '+'}{formatCurrency(item.delta)}</p><p className="mt-1 text-[9px] font-medium text-[#718092]">{formatCurrency(item.previousPrice)} → {formatCurrency(item.latestPrice)}</p></div></div> })}</div></div><div className="strong-panel p-5"><p className="eyebrow">Snelle acties</p><h2 className="mt-1 text-[17px] font-bold text-[#243247]">Wat wil je nu doen</h2><div className="mt-4 grid gap-2"><Link href="/producten" prefetch className="primary-action justify-between">Prijsafwijkingen openen <span>→</span></Link><Link href="/productmatches" prefetch className="secondary-action justify-between">EAN en match suggesties <span>→</span></Link><Link href="/monitoring" prefetch className="secondary-action justify-between">Monitoringstatus <span>→</span></Link><Link href="/import" prefetch className="secondary-action justify-between">Producten importeren <span>→</span></Link></div></div></section>
    <section className="flex flex-wrap items-center gap-2 rounded-[10px] border border-[#d9e0e7] bg-white px-4 py-3 shadow-[0_1px_3px_rgba(16,28,44,.035)]"><span className="mr-1 text-[10px] font-semibold uppercase tracking-[.08em] text-[#718092]">Markt</span><Link href="/dashboard" prefetch className={`rounded-[7px] border px-3 py-2 text-[10px] font-semibold ${!filters.countryId ? 'border-[#1769aa] bg-[#1769aa] text-white' : 'border-[#d7dee5] text-[#526173]'}`}>Alle landen</Link>{snapshot.filterOptions.countries.map((country) => <Link key={country.id} href={`/dashboard?land=${country.id}`} prefetch className={`rounded-[7px] border px-3 py-2 text-[10px] font-semibold ${filters.countryId === country.id ? 'border-[#1769aa] bg-[#1769aa] text-white' : 'border-[#d7dee5] text-[#526173] hover:border-[#9fb2c2]'}`}>{country.code}</Link>)}</section>
  </div>
}
