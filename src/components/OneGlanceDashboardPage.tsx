...
function StatCard({ label, value, href, accent }: {
  label: string; value: string; href: string; accent: 'neutral' | 'red' | 'amber' | 'green'
}) {
  const accentStyles = {
  ...
  return <Link href={href} className="group min-h-[108px] rounded-[14px] border border-[#e4eaf2] bg-white p-4 shadow-sm transition hover:border-[#b8cce9]">
...
export default async function OneGlanceDashboardPage({ searchParams }: {
  ...
  return <div className="space-y-4 pb-6">
    ...
    <section aria-label="Belangrijkste cijfers" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <StatCard label="Datadekking" value={coveragePct + '%'} href={productHref} accent={coveragePct >= 90 ? 'green' : coveragePct >= 70 ? 'neutral' : 'amber'}/>
      <StatCard label="Boven marktprijs" value={formatNumber(higher.length)} href={productHref} accent="red"/>
      <StatCard label="Gemiddelde prijsafwijking" value={avgDeviation === null ? '—' : (avgDeviation > 0 ? '+' : '') + formatNumber(avgDeviation, 1) + '%'} href={productHref} accent={avgDeviation !== null && avgDeviation > 0 ? 'amber' : 'green'}/>
      <StatCard label="Openstaande signalen" value={formatNumber(signals)} href="/waarschuwingen" accent="amber"/>
    </section>
